...
const SearchBar = () => {
    ...
    const findBooks = async (query: string) => {
        dispatch(load(true));
        query.replace(/ /g, "+");
        const res = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${query}&orderBy=${sortState}&maxResults=30&fields=items(id,volumeInfo/title,volumeInfo/authors,volumeInfo/description,volumeInfo/categories,volumeInfo/imageLinks/thumbnail)&key=${import.meta.env.VITE_BOOKSAPI}`);
    ...
    return (
        <form className="w-full my-5" onSubmit={handleSubmit}>
            <div className="relative block">
                <input
                    className="bg-white border-stone-300 border caret-stone-600 text-xl text-stone-600 w-full rounded-xl py-2 sm:pl-28 pl-16 pr-10 focus:outline-none"
                ...
                <button
                    ...
                    type="submit">
                    {
                        loadingState
                            ? <Spinner size={"small"} />
                            :
                            <img src={searchIcon} alt="Search icon" className="opacity-40" />
                    }
                </button>
                <select
                    className="border-stone-300 border cursor-pointer text-stone-600 absolute inset-y-0 left-0 w-1 sm:w-fit bg-white rounded-l-xl pl-3 pr-3 focus:outline-none"
